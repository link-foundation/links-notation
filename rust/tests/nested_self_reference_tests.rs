...
fn test_nested_self_referenced_object_in_pair_value() {
    ...
    if let LiNo::Link { id, values } = &links[0] {
        ...
        if let LiNo::Link { id: pair1_id, values: pair1_values } = pair1 {
            assert_eq!(pair1_id, &None);
            assert_eq!(pair1_values.len(), 2);

            // First element of pair1: (str bmFtZQ==)
            if let LiNo::Link { id: elem1_id, values: elem1_values } = &pair1_values[0] {
            ...
            if let LiNo::Link { id: elem2_id, values: elem2_values } = &pair1_values[1] {
        ...
        if let LiNo::Link { id: pair2_id, values: pair2_values } = pair2 {
            assert_eq!(pair2_id, &None);
            assert_eq!(pair2_values.len(), 2);

            // First element of pair2: (str b3RoZXI=)
            if let LiNo::Link { id: key_id, values: key_values } = &pair2_values[0] {
            ...
            if let LiNo::Link { id: obj1_id, values: obj1_values } = &pair2_values[1] {
                assert_eq!(obj1_id, &Some("obj_1".to_string()), "obj_1 should have its ID preserved");
                assert_eq!(obj1_values.len(), 3, "obj_1 should have 3 values (type marker + 2 pairs)");

                // obj_1's type marker
                assert_eq!(obj1_values[0], LiNo::Ref("dict".to_string()));

                // obj_1's first pair: ((str bmFtZQ==) (str ZGljdDI=))
                if let LiNo::Link { values: obj1_pair1_values, .. } = &obj1_values[1] {
                    assert_eq!(obj1_pair1_values.len(), 2);

                    if let LiNo::Link { values: k1_values, .. } = &obj1_pair1_values[0] {
                        assert_eq!(k1_values[0], LiNo::Ref("str".to_string()));
                        assert_eq!(k1_values[1], LiNo::Ref("bmFtZQ==".to_string()));
                    }

                    if let LiNo::Link { values: v1_values, .. } = &obj1_pair1_values[1] {
                ...
                if let LiNo::Link { values: obj1_pair2_values, .. } = &obj1_values[2] {
                    assert_eq!(obj1_pair2_values.len(), 2);

                    if let LiNo::Link { values: k2_values, .. } = &obj1_pair2_values[0] {
...
fn test_self_reference_as_direct_child_works_correctly() {
    ...
    if let LiNo::Link { id, values } = &links[0] {
        ...
        if let LiNo::Link { id: obj1_id, values: obj1_values } = &values[3] {
