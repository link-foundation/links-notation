...
spl_autoload_register(static function (string $class): void {
    ...
    if (str_starts_with($class, $prefix)) {
        require __DIR__ . '/../php/src/' . substr($class, strlen($prefix)) . '.php';
    }
});
...
$parser = new LinkFoundation\LinksNotation\Parser();
foreach ($cases as [$source, $expected]) {
    $actual = LinkFoundation\LinksNotation\Formatter::formatLinks($parser->parse($source));
    if ($actual !== $expected) {
        throw new RuntimeException("Expected $expected, got $actual");
    }
}
echo "6 issue #313 cases passed\n";
