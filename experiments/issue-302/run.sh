#!/usr/bin/env bash
# Asks every implementation what it says about the same five documents, four of
# which do not parse. Written for https://github.com/link-foundation/links-notation/issues/302:
# the answers are what the issue is about, and what the parity gap in the
# implementations that accept all five looks like.
#
...
if command -v cargo >/dev/null; then
  cargo run --quiet --manifest-path rust/links-notation/Cargo.toml \
    --example parse_error_positions
else
