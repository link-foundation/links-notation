...
func TestIndentedIDRetainsNestedValues(t *testing.T) {
	cases := []struct{ source, expected string }{
	...
	for _, tc := range cases {
		actual, err := NewParser().Parse(tc.source)
		if err != nil {
			t.Fatalf("%q: %v", tc.source, err)
		}
		if got := Format(actual); got != tc.expected {
			t.Errorf("%q: got %q, want %q", tc.source, got, tc.expected)
		}
	}
}
