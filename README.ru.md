...
Каждая поддерживаемая реализация умеет разбирать произвольные порции входных
данных, включая посимвольную и построчную подачу. Связь верхнего уровня
выдаётся только после завершения её дочерних строк с отступами, многострочных
ссылок в кавычках и контекстов в скобках. Каждую завершённую запись проверяет
основной парсер, поэтому потоковый разбор использует тот же синтаксис и даёт тот
же результат, что и разбор целого документа.

| Язык | Push API | Ленивый/нативный адаптер |
| --- | --- | --- |
| C# | событие `LinkParsed` | `IEnumerable` / `IAsyncEnumerable` |
| JavaScript | события `link`, `error` и `end` | синхронные / асинхронные итераторы |
| Rust | callback-и связей и ошибок | `Iterator` |
| Python | callback связи | генератор / асинхронный генератор |
| Go | callback и `io.Writer` | `iter.Seq2` |
| Java | `Consumer<Link>` | `Stream<Link>` |
| PHP | callable | `Generator` |

Все варианты поддерживают запись/подачу порций, завершение потока, абсолютную
позицию, reset/drain и ограничение размера незавершённой записи. По умолчанию
связи накапливаются; отключите накопление, чтобы callback-и или ленивый
итератор работали с ограниченной памятью. См. [эксперимент проверки паритета](experiments/issue-197/README.md)
и README соответствующего языка.
